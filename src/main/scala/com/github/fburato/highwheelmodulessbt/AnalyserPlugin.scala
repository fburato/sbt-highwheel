...
import sbt.Keys._
import sbt._
import com.github.fburato.highwheelmodules.core._
...
object AnalyserPlugin extends AutoPlugin {
  ...
  object autoImport {
    ...
    val highwheelEvidenceLimit = settingKey[Option[Int]](
      "Amount of dependencies to show in case of error, None for all the evidence"
    )
  }
  ...
  override lazy val projectSettings = Seq(
    ...
    highwheelBaseAnalyseTask := {
      ...
      Analyser(
        log,
        highwheelSpecFiles.value,
        highwheelAnalysisPaths.value,
        highwheelEvidenceLimit.value
      )
    },
...
object Analyser {
  ...
  def apply(
    log: ManagedLogger,
    specFiles: Seq[File],
    analysisPaths: Seq[File],
    evidenceLimit: Option[Int]
  ): Unit = {
    val paths = specFiles.map(_.getAbsolutePath())
    log.info(s"Using specification files: '${paths.mkString(",")}'")

    val facade = new AnalyserFacadeImpl(
      printer(log),
      pathSink(log),
      measureSink(log),
      strictAnalysisSink(log, evidenceLimit),
      looseAnalysisSink(log, evidenceLimit)
    )
    facade.runAnalysis(
      new util.ArrayList(analysisPaths.map { f => f.getAbsolutePath }.asJavaCollection),
      new util.ArrayList(paths.asJavaCollection),
      evidenceLimit.map(Integer.valueOf).asJava
    )
  }
  ...
  private def strictAnalysisSink(
    log: ManagedLogger,
    evidenceLimit: Option[Int]
  ): StrictAnalysisEventSink = new StrictAnalysisEventSink {
    override def dependencyViolationsPresent(): Unit =
      log.error("The following dependencies violate the specification:")

    override def noDirectDependenciesViolationPresent(): Unit =
      log.error("The following direct dependencies violate the specification:")
    ...
    override def directDependenciesCorrect(): Unit =
      log.info("No direct dependency violation detected")
    ...
    override def dependencyViolation(
      ...
      usagePath: util.List[util.List[Pair[String, String]]]
    ): Unit = {
      log.error(
        f"  $sourceModule%s -> $destModule%s. Expected path: ${pathToString(expectedPath)}%s, Actual module path: ${pathToString(actualPath)}%s"
      )
    ...
    override def noDirectDependencyViolation(source: String, dest: String): Unit =
      log.error(s"  $source -> $dest")
  }

  private def looseAnalysisSink(
    log: ManagedLogger,
    evidenceLimit: Option[Int]
  ): LooseAnalysisEventSink = new LooseAnalysisEventSink {
    ...
    override def absentDependencyViolation(source: String, dest: String): Unit =
      log.error(s"  $source -> $dest")

    override def absentDependencyViolationsPresent(): Unit =
      log.error("The following dependencies do not exist:")
    ...
    override def undesiredDependencyViolation(
      ...
      log.error(
        f"  $sourceModule%s -> $destModule%s, Actual module path: ${pathToString(path)}%s\n"
      )
      def logEvidence(): Unit = {
        log.error("    Actual usage path:")
  ...
  private def printEvidence(
    log: ManagedLogger,
    modulePath: util.List[String],
    usagePath: util.List[util.List[Pair[String, String]]]
  ): Unit = {
    @tailrec
    def scalaVersion(
      modulePath: List[String],
      connections: List[List[Pair[String, String]]]
    ): Unit = (modulePath, connections) match {
